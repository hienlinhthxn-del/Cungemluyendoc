...
export const TeacherDashboard: React.FC = () => {
  ...
  const handleDeleteStudent = async (id: string, name: string) => {
    ...
    if (window.confirm(`Bạn có chắc chắn muốn xóa học sinh ${name} khỏi lớp không?`)) {
      // 1. Giữ lại state gốc để có thể khôi phục nếu có lỗi
      ...
      try {
        ...
        // Nếu có lỗi mạng, cũng khôi phục lại
        setNotification({ message: `Lỗi mạng: không thể xóa học sinh ${name}. Vui lòng thử lại.`, type: 'error' });
        setStudents(originalStudents);
      }
    }
  };
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative inline-block">
              <select
                value={selectedWeek}
                onChange={(e) => { playClick(); setSelectedWeek(Number(e.target.value)); }}
                className="appearance-none bg-blue-50 border border-blue-200 text-blue-800 font-bold py-1.5 px-4 pr-10 rounded-xl cursor-pointer hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all shadow-sm"
              >
                {/* Dynamically list all available weeks found in lessons */}
                {weeks.map(w => (
                  <option key={w} value={w}>Tuần {w}</option>
                ))}
              </select>
              <ChevronDown className="w-5 h-5 text-blue-600 absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" />
            </div>

            <button
              onClick={handleSync}
              className={`p-2 rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors ${isSyncing ? 'animate-spin' : ''} border border-blue-100 shadow-sm`}
